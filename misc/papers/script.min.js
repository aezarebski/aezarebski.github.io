...
if(outDegreeMap.hasOwnProperty(link.target)){outDegreeMap[link.target]+=1;}});console.log(outDegreeMap);data.nodes.forEach(node=>{node.radius=baseRadius+(outDegreeMap[node.id]+1);});const simulation=d3.forceSimulation(data.nodes).force("link",d3.forceLink(data.links).id(d=>d.id).distance(linkLength)).force("charge",d3.forceManyBody().strength(-800)).force("center",d3.forceCenter(width/2,height/2));const link=g.append("g").attr("class","links").selectAll("line").data(data.links).enter().append("line").attr("class","link").style("stroke-opacity",defaultOpacity);const node=g.append("g").attr("class","nodes").selectAll("circle").data(data.nodes).enter().append("circle").attr("class","node").attr("r",d=>d.radius).style("fill","grey").style("opacity",defaultOpacity).on("mouseover",function(event,d){const neighbors=new Set(data.links.filter(l=>l.source.id===d.id||l.target.id===d.id).flatMap(l=>[l.source.id,l.target.id]));neighbors.add(d.id);node.style("opacity",n=>neighbors.has(n.id)?1:defaultOpacity);link.style("stroke-opacity",l=>neighbors.has(l.source.id)&&neighbors.has(l.target.id)?1:defaultOpacity);const xPosition=event.pageX+5;const yPosition=event.pageY+5;d3.select("#tooltip").style("left",xPosition+"px").style("top",yPosition+"px").style("opacity",1);d3.select("#tooltip-title").text(d.title);d3.select("#tooltip-year").text(d.year);d3.select("#tooltip-first-author").text(d.first_author);d3.select("#tooltip-doi").attr("href","https://doi.org/"+d.doi).text(d.doi);}).on("click",function(event,nodeData){addToTooltipTable(nodeData,data);nodeData.addedToTable=true;d3.select(this).style("fill","blue");}).on("mouseout",function(event,nodeData){node.style("opacity",defaultOpacity);link.style("stroke-opacity",defaultOpacity);setTimeout(()=>{if(!d3.select("#tooltip").classed("hovered")){d3.select("#tooltip").style("opacity",0).style("left","-9999px");}},1000);});d3.select("#tooltip").on("mouseover",function(){d3.select(this).classed("hovered",true);}).on("mouseout",function(){d3.select(this).classed("hovered",false);setTimeout(()=>{if(!d3.select(this).classed("hovered")){d3.select(this).style("opacity",0).style("left","-9999px");}},1000);});simulation.on("tick",()=>{link.attr("x1",d=>d.source.x).attr("y1",d=>d.source.y).attr("x2",d=>d.target.x).attr("y2",d=>d.target.y);node.attr("cx",d=>d.x).attr("cy",d=>d.y);});});function zoomed(event){g.attr("transform",event.transform);}
function addToTooltipTable(node,graphData){if(node.addedToTable){return;}
var tooltipTable=document.getElementById("tooltipTable");var tableRow=document.createElement("tr");tableRow.innerHTML=`<td><button class="center-node-btn"data-id="${node.id}">Center</button></td><tb><button class="add-neighbours-btn"data-id="${node.id}">Add neighbours</button></td><td>${node.title}</td><td>${node.year}</td><td>${node.first_author}</td><td><a href="https://doi.org/${node.doi}"target="_blank">${node.doi}</a></td><td><button class="remove-node-btn"data-id="${node.id}">Remove</button></td>`;tooltipTable.appendChild(tableRow);tableRow.querySelector(`.center-node-btn[data-id='${node.id}']`).addEventListener("click",function(){centerNode(node);});tableRow.querySelector(`.add-neighbours-btn[data-id='${node.id}']`).addEventListener("click",function(){if(!graphData||!graphData.links){console.warn("Graph data is not available.");return;}
let neighbors=graphData.links.filter(link=>link.source.id===node.id||link.target.id===node.id).map(link=>link.source.id===node.id?link.target:link.source);neighbors.forEach(neighbor=>{if(!neighbor.addedToTable){addToTooltipTable(neighbor,graphData);neighbor.addedToTable=true;}});});tableRow.querySelector(`.remove-node-btn[data-id='${node.id}']`).addEventListener("click",function(){tooltipTable.removeChild(tableRow);node.addedToTable=false;d3.selectAll(".node").filter(n=>n.id===node.id).transition().duration(750).style("fill","grey");});}
function centerNode(d){const svg=d3.select("svg");const g=d3.select("g");const transform=d3.zoomTransform(svg.node());const centerX=width/2;const centerY=height/2;const newX=centerX-d.x*transform.k;const newY=centerY-d.y*transform.k;svg.transition().duration(750).call(d3.zoom().transform,d3.zoomIdentity.translate(newX,newY).scale(transform.k));g.transition().duration(750).attr("transform",`translate(${newX},${newY})scale(${transform.k})`);d3.selectAll(".node").filter(n=>n.id===d.id).transition().duration(750).style("fill","red");};